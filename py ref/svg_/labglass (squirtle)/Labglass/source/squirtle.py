...
class SVG(object):
    ...
    def draw(self, x, y, z=0, angle=0, scale=1):
        ...
        for text in self.textList:
            #textGfx = pyglet.text.Label(text[0][1],'Arial',font_size=text[3][0]-2, x=text[1], y=text[2],color=(0,0,0,255))
            textGfx = pyglet.text.Label(self.text,'Arial',font_size=text[3][0]-2, x=text[1], y=text[2],color=(0,0,0,255))
    ...
    def parse_element(self, e):
        ...
        ####added text handling
            ...
            fweight=""
            ffam=""
            fontinfo=e.get('style').split( ";" )
            for finfo in fontinfo:
                if finfo.find("font-size:") > -1:
                    #print("font size",finfo[10:])
                    fsize=float(finfo[10:10+finfo[10:].index("px")])
                if finfo.find("font-style:") > -1:
                    #print("font style",finfo[11:])
                    fstyle=finfo[11:]
                if finfo.find("font-weight:") > -1:
                    #print("font weight",finfo[12:])
                    fweight=finfo[12:]
                if finfo.find("font-family:") > -1:
                    #print("font family",finfo[12:])
                    ffam=finfo[12:]

            style=(fsize,fstyle,fweight,ffam)
            #print(style)
            


            tmp=e.find('{http://www.w3.org/2000/svg}tspan')
            tspan=''
            if tmp is not None :
                #print(tmp.text)
                #print(tmp.get('id'))
            ...
            truc=(tspan,x,y,style,id)
            self.textList.append(truc)
            #print(truc)
        ####

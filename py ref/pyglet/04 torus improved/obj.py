...
'''
Since Obj.py, the original pyglet objloader is not supplied with the release
I am using, I include this version....again slightly changed  (Jestermon))
Obj Loader from the Pyweek #4  Game delta-v by Alex Holkner
...
# A better OBJ loader, based on pyglet.ext.model.obj
...
class Mesh(object):
    def __init__(self, name):
        ...
        # Not used for rendering, but better for collision
        self.polygons = []
        ...
        for line in open(filename, "r"):
            ...
            if values[0] == 'v':
                ...
                for i, v in enumerate(values[1:]):
                    ...
                    points.append(euclid.Point3(*vertices[v_index]))
                mesh.polygons.append(collide.ConvexPolygon(points))
            ...
            try:
                if values[0] == 'Kd':
                    ...
                    try:
                        ...
                        try:
                            material.texture = image.load('data/'+values[1]).texture
                        except:

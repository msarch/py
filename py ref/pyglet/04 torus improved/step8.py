...
step8.py
-----------------
Step 8 of in_progress
a..changed on_draw() show how not resetting the viewport with glLoadIdentity()
   will keep the last offsets and rorations, and use the first cube's values
   as a base for the ogl rendering..
   Notice how the cubes move closer together as the central cube moves closer
   to the 0,0,0 point of the screen. All the values are translated from 1st to
   second cube
...
class World(pyglet.window.Window):
    ...
    def on_draw(self):
        ...
        self.box.draw()
        #2nd copy of the cube drawn
        #based on the values of the previous translation an reotation
        glTranslatef(0, 0, self.z)
        glRotatef(self.rz, 0, 0, 1)
        glRotatef(self.ry, 0, 1, 0)
        glRotatef(self.rx, 1, 0, 0)

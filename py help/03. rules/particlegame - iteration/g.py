...
class GameScene(Scene):
        ...
        for row in level:
            for col in row:
                ...
                if col == "E":
                    ...
                    self.entities.add(e)
                if col == "S":
                    self.emitter['bg'].append(Emitter((x, y+32), smoke_machine(total_level_width, total_level_height)))
                if col == "R":
                    self.emitter['fg'].append(Emitter((x, y), rain_machine(total_level_width, total_level_height)))
                if col == "W":
                    self.emitter['fg'].append(Emitter((x, y), wind_machine(total_level_width, total_level_height)))
                x += 32
            y += 32
            x = 0
    ...
    def render(self, screen):
        screen.fill(Color("#002233"))

        for e in self.emitter['bg']:
            e.draw(screen, self.camera.apply)

        for e in self.entities:
            screen.blit(e.image, self.camera.apply(e))

        for e in self.emitter['fg']:    
            e.draw(screen, self.camera.apply)
        
    def update(self):
        for g in self.emitter.values():
            for e in g:
                e.update()

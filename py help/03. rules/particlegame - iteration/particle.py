...
def rain_machine(total_level_width, total_level_height):
    ...
    behaviour = ascending(-6), wind(4, 80), translate_x(total_level_width), kill_at(total_level_width, total_level_height)
...
def smoke_machine(total_level_width, total_level_height):
    ...
    def create():
        for _ in xrange(numpy.random.choice([0,0,0,0,0,0,0,1,2,3])):
            c = cm[numpy.random.choice(range(len(cm)))]

...
class Point(Grob):
    """ Point (x, y, M)
                    ...
                    y = y position : float
                    c = color (rgb) list
                    M = current transform martix : 3 x 3 matrix
    """
    ...
    def transform(self):
        """ applies the current matrix transformation.
        """
        self.x = self.M[0]*self.x+self.M[1]*self.y+self.M[2]
        self.y = self.M[3]*self.x+self.M[4]*self.y+self.M[5]
    ...
    def pygdraw(self): #point is drawn as a 3 pixel rectangle on screen
        ...
        glBegin(GL_QUADS)
        glVertex3f(self.x-1, self.y-1, 0.0)    # bottom left
        glVertex3f(self.x+1, self.y-1, 0.0)    # bottom right
        glVertex3f(self.x+1, self.y+1, 0.0)    # top right
        glVertex3f(self.x-1, self.y+1, 0.0)    # top left
        glEnd()
...
# TODO déplacer transform au niveau de la super-classe grob
# TODO creer 3 transform : transforma, b, w normal, avec bounce avec wrap
    ...
    def transform(self):
        """ applies the current matrix transformation.
        """
        self.x0 = self.M[0]*self.x0+self.M[1]*self.y0+self.M[2]
        self.y0 = self.M[3]*self.x0+self.M[4]*self.y0+self.M[5]
        self.x1 = self.M[0]*self.x1+self.M[1]*self.y1+self.M[2]
        self.y1 = self.M[3]*self.x1+self.M[4]*self.y1+self.M[5]
...
class Rect(Grob):
    ...
    def __init__(self, width=300, height=100, xc=0, yc=0, \
        ...
        # self vertex list:
        # centroid, bottom left, bottom right, top right,top left
    ...
    def transform(self):
        """ applies the current matrix transformation
        to all vertex, incl the centroid.
        """
    ...
    def m_transform(self,M):
        """ applies the provided matrix transformation to all vertex.
        """
        for index, vtx in enumerate(self.v):

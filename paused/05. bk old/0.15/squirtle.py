...
# my adapatation to squirtle : getsvg() function
# will load svg from local 'svg' subfolder
# calling files with just their name+ext
#   ex : getsvg(mysketch.svg)
mydir=os.path.dirname(os.path.abspath(__file__))
svgdir=os.path.join(mydir,'svg')

def getsvg(name):
    global svgdir
    filename=os.path.join(svgdir,name)
    return SVG(filename)
...
class SVG(object):
	...
	def parse_element(self, e):
		...
		display = e.get('display')#NEW CODE TAG TEST
		visibility = e.get('visibility')#NEW CODE TAG TEST

...
# who : ms
# when : 04.2013
# what : tools for movement on a 2d plane
...
# TODO : a finir, reflechir à unifier les représentations des elts geom avant.

    def border_check(BoxA,BoxB):
        """ Check vertex is in, touching border or out
        to all transformable vertex, incl the centroid.
        """
        # for this version : we suppose H or V movement only
        global canvas
        for index, vtx in enumerate(self.v):
            if abs(self.v[index][0]) > 200: print 'sortie x'
            if abs(self.v[index][1]) > 300: print 'sortie y'
        return(status)



#
#  2D AFFINE TRANSFORMATION MATRIX FUNCTIONS
#------------------------------------------------------------------------------
#        [a b c]
#        [d e f]
#        [g h i]
# is represented as a simple list of nine elements :
...
# TODO : a tester
def general_rotation_matrix(x,y, alpha):
    """ rotation matrix, from another
        center=object's centroid ??????????
